...
node {
  ...
  try {
    ...
    stage('Scan Packages') {
      snykSecurity severity: 'medium', snykInstallation: 'snyk-security-scanner', snykTokenId: 'Snyk-Token', targetFile: './FFCDemoPaymentService/FFCDemoPaymentService.csproj', packageManager: 'nuget'
    }
    stage('Run tests') {
