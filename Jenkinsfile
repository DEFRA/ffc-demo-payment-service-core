...
node {
  ...
  try {
    ...
    stage('Scan Packages') {
      snykSecurity projectName: 'ffc-demo-payment-service-core', severity: 'medium', snykInstallation: 'snyk-security-scanner', snykTokenId: 'Snyk-Token', targetFile: '${workspace}/FFCDemoPaymentService/FFCDemoPaymentService.csproj'
    }
    stage('Helm lint') {
